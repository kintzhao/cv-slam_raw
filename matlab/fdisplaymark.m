...
figure(num)
[hang,lie] = size(marks);
title('r g b y k 0 1 2 3 middle');
axis equal;
plot(0,0,'r+');hold on;
...
for row = 500:3:hang
    ...
    for num = 1:k
        ...
        plot(marks(row, 2+11*(num-1)+7),marks(row, 2+11*(num-1)+8),'y*'); hold on;
        line( [marks(row, 2+11*(num-1)+7), marks(row, 2+11*(num-1)+1) ],[ marks(row, 2+11*(num-1)+8), marks(row, 2+11*(num-1)+2) ] );hold on;
        
        plot(marks(row, 2+11*(num-1)+9),marks(row, 2+11*(num-1)+10),'k-');hold on;
    end
end
grid on;
