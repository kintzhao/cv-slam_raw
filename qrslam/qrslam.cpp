...
QrSlam::QrSlam(char* addr,char* config):transport_( ar_handle)
{
    ...
    sync_->registerCallback(boost::bind(&QrSlam::combineCallbackNew,this, _1, _2));
    ...
    fmiu << " robot_odom_.x  .y .theta << miu(3N)) x y theta    "  ;
...
//========================= 配置信息 ==========================
void QrSlam::readConfigure(char* config)
{
...
//============================== SLAM ===========================
void QrSlam::combineCallbackNew(const nav_msgs::Odometry::ConstPtr& pOdom, const sensor_msgs::ImageConstPtr& pImg)  //回调中包含多个消息
{
 ...
 * @return
 */
void QrSlam::stateExtended(const int curr_num, const int num_old)
{
...
void QrSlam::ekfslamNew(OdomMessage rob_odom, Mat& img)
{
    ...
    {
        float time_diff = (image_stamp_ - time_bag_old).toSec(); //  秒
        ...
        if (1)  //运动触发
        {
            ...
            if(!marks_pix.empty())   //  提取到二维码图像处理
            {
                ...
                frameFixedTransform(miu_state_, Fixed_mark_ID_);
                ...
                robot_node.pic_Infs = pic_node;    // 节点  图像块
 ...
 */
Point3f QrSlam::motionModelNew(Point2f motion, Mat& system_state, Mat& system_state_convar, int variable_num, float delta_time)
{
    Point2f VEL;
    VEL.x = motion.x;
    ...
    robot_increase.x = miu_increase.at<float>(0) - last_rob_pose_.x ;
    robot_increase.y = miu_increase.at<float>(1) - last_rob_pose_.y ;
    robot_increase.z = miu_increase.at<float>(2) - last_rob_pose_.z ;

    return robot_increase;
}

int QrSlam::findLandmarkId(int id)
{
...
OdomMessage QrSlam::getOdomDataNew(const nav_msgs::Odometry::ConstPtr& odomMsg)
{
...
bool QrSlam::addInitPos(cv::Mat img, const OdomMessage robot_state, const int mark_ID, const int num_FULL)
{
    bool IS_FULL = false;
    ...
    if ( abs(robot_state.x)<2 && abs(robot_state.y)<2 )  //起始 2cm 以内
    {
...
cv::Mat QrSlam::getSystemState(void )
{
    return miu_state_;
}
Point3f QrSlam::initFrameDiff(const vector<CPointsFour> init_marks_metric,const CPointsFourWorld origin_world)
{
...
vector<CPointsFour>  QrSlam::imgProcessNew( cv::Mat& img, bool& Is_Gobal_Search, Point3f& best_mark_info, SearchInfo& search_info)
{
...
vector<Point4fffi> QrSlam::getObservationsNew( vector<CPointsFour> marks_metric )
{
...
Point4fffi QrSlam::addObservationPosNew(CPointsFour code_one ,int id )
{
    Point2f delta;
    float dst;   //特征距离
    ...
    mark_one.theta = 1.0*(  atan2(  (code_one.corn3.Y - code_one.corn2.Y ),(code_one.corn3.X - code_one.corn2.X ))
                             +atan2( (code_one.corn0.Y - code_one.corn1.Y ),(code_one.corn0.X - code_one.corn1.X ))
                             )/2;
    return mark_one;
}
int  QrSlam::getLandmarkNumNew(const vector<Point4fffi> curr_marks_observed, vector<int>& landmark_ids)
{
...
void QrSlam::updateSLAM(Mat& sys_state, Mat& sys_convar, int landmarks_num, vector<Point4fffi>& curr_marks, vector<int>& index_landmarks)
{
    Mat Q_convar_ = Mat::zeros(2, 2, CV_32FC1);
    ...
    for (int i=0; i<curr_marks.size(); i++)
    {
        ...
        for (int index=0; index<index_landmarks.size(); ++index)
        {
            ...
            {
                int curr_landmark_index = index;                     //  选取第j个landmark导入观察模型并进行状态更新。
                ...
                angleWrap(sys_convar.at<float>(2));
                angleWrap(sys_convar.at<float>(3 * curr_landmark_index+5));
                sys_convar =  (Identity_system - Kt * H_system) * sys_convar;
            }
            else
            {
                index_landmarks.push_back(select_mark_id) ;
                int curr_landmark_index = index_landmarks.size()-1 ;
...
//                    Fixed_landmark_index_ = curr_landmark_index;
        ...
        }//end for  index_landmarks
    }//end for  curr_marks
    sys_convar =  0.5*(sys_convar + sys_convar.t());
}

vector<int>  QrSlam::getLandmarksIndex()
{
      return   index_landmarks_;
}

int  QrSlam::getFixedLandmarkIndex(const int fixed_mark_id)
{
...
int QrSlam::getLandmarksNum()
{
    return index_landmarks_.size();
}

void QrSlam::getOdomDataByAcc(const nav_msgs::Odometry::ConstPtr& odomMsg)
{
    static bool init_flag_stamp = true;  //第一次记录参考时间戳
...
Point3f QrSlam::odomModel(Point2f motion, Point3f robotPose,float delta_time)
{
...
cv::Mat QrSlam::getCovMatrix(const cv::Mat state_matrix, int i, int j)
{
...
//===================  闭环检测 与 g2o优化 ===========================
PicNode QrSlam::getPicInfo(const vector<CPointsFour>  marks_pix)
{
    PicNode pic_node;
...
int QrSlam::checkLoopClosure(const vector<RobNode> history_nodes,const PicNode current_pic_node, int num, int min_score_pic, int min_value_mark)
{//TODO
    bool IS_MATCH = false;
    for(int i=0; i<num; i++)  //随机次数
    {
        int k = rand()/(RAND_MAX + 1.0)*(history_nodes.size()-1);
        if( k >= history_nodes.size()) return -1;
        RobNode history_node = history_nodes.at(k);
        int score = getMatchScore(history_node.pic_Infs, current_pic_node, min_value_mark ); //获取两张图片的匹配分数
...
void QrSlam::setG2oInfo(const Mat sys_state, const int index_g2o_node)
{
...
void QrSlam::graphOptimization()
{
    // 优化所有边
    cout<<"optimizing pose graph, vertices: "<<optimizer_.vertices().size()<<endl;
    optimizer_.save("./data/result_before.g2o");
    optimizer_.initializeOptimization();
    optimizer_.optimize( 100 ); //可以指定优化步数
    optimizer_.save( "./data/result_after.g2o" );
    cout<<"Optimization done."<<endl;
    optimizer_.clear();
}

Eigen::Matrix3d QrSlam::getRobotInformation()
{
...
//====================图像处理==================
vector<CPointsFour> QrSlam::imgDetectMarks(cv::Mat image )
{
    vector<CPointsFour> Marks_pix = pQrDetect_->detectMarks(image, visual_mark_num_, true);
    return Marks_pix;
}
vector<CPointsFour> QrSlam::imgDetectMarks(cv::Mat image, Point3f mark_center)
{
...
vector<CPointsFour> QrSlam::pixToMetric(vector<CPointsFour> pix_vector)
{
...
cv::Mat QrSlam::inverseObservationModel(cv::Mat& full_img, Point3f& mark_center, Mat system_state)
{
    Point3f pos_world ;
...
int QrSlam::getMatchScore(PicNode history_pic_node, PicNode current_pic_node, int min_distance)
{
    int score = 0;
...
bool QrSlam::evaluateNew(vector<CPointsFour> marks_pix, Point3f& best_mark_center)
{
    float min = 800.0;
    float Radius ;
    for(int i=0; i<marks_pix.size();i++)
 ...
 * @param imgMsg
 */
cv::Mat QrSlam::getImgDataNew(const sensor_msgs::ImageConstPtr& imgMsg)
{
...
//=====================绘图相关==================
void QrSlam::showImageNew()
{
    showRobot(raw_global_map_, robot_odom_, CV_RGB(0,0,0)) ;

    showSystemStateRobotNew(raw_global_map_,CV_RGB(0,0,255));
    ...
    showSystemStateLandmarkNew( global_map_display_, CV_RGB(0,0,255));
...
void QrSlam::showSystemStateRobotNew(cv::Mat& map, Scalar rgb)
{
    int temp_X = miu_state_.at<float>(0) + 1000;
    int temp_Y = miu_state_.at<float>(1) + 1000;
    temp_Y = map.rows - temp_Y ;
    cv::circle(map,Point( temp_X,temp_Y),1,CV_RGB(0, 255,0),1); //绘制 robot

    cv::Mat rob_motion_cov = getCovMatrix(miu_state_, 0, 1);
    cv::Point2f center(temp_X,temp_Y);
    drawEllipse(map, center, rob_motion_cov, CV_RGB(0, 150,0) );
}

void QrSlam::showSystemStateLandmarkNew(cv::Mat& map, Scalar rgb)
{
    // cv::Mat map_copy ;
    for (int t = 0; t < id_all_marks_.size(); t++)
    {
        float X= miu_state_.at<float>(3+t*3)+ 1000;
        float Y= miu_state_.at<float>(4+t*3)+ 1000;

        Y = map.rows - Y ;
        cv::circle(map,Point( X,Y),1,rgb,2); //绘制mark位置

        cv::Mat landmark_cov = getCovMatrix(miu_state_, 3+t*3, 4+t*3);
...
void QrSlam::showRobotOrientationNew(Mat image, OdomMessage rob_odom,Scalar rgb,int x_coordinate,int y_coordinate)
{
...
void QrSlam::showRobotOrientationNew(Mat image, Mat rob_update_mat,Scalar rgb,int x_coordinate,int y_coordinate)
{
    const int ROBOT_DEFAULT_RADIUS = 2;
    const int ROBOT_DEFAULT_ARROW_LEN = 30;
...
void QrSlam::writeMatrix(Mat matrix ,ofstream& f_test)
{
    for (int ii=0;ii<matrix.rows;ii++)
    {
        for (int jj=0;jj<matrix.cols;jj++)
...
void QrSlam::showRobot(cv::Mat& map, OdomMessage rob_odom,Scalar rgb)
{
    ...
    Point start, end;
    start.x = robot_pose.x + map_base_y_;
    start.y = robot_pose.y + map_base_x_;
...
void QrSlam::showRobotTriangle(cv::Mat& map, OdomMessage rob_odom, Scalar rgb)
{
    ...
    robot_pos.y = map.rows - (rob_odom.y + map_base_x_);
...
void QrSlam::drawEllipse(cv::Mat img_test, cv::Point2f center, cv::Mat covMartix, const Scalar& color)
{
    Mat eigen_values;
    Mat eigen_vector;
...
//=====================数据保存==================
void QrSlam::saveMarksPerImgPix( vector<CPointsFour> vector_data, ofstream&  file )
{
...
void QrSlam::saveMarksPerImgMetric( vector<CPointsFour> vector_data,ofstream&  file )
{
    file  <<  vector_data.size();
    for (vector<CPointsFour>::size_type i=0; i< vector_data.size();i++)
    {
...
void QrSlam::saveMeterDatas( vector<CPointsFour> vector_data,ofstream&  writeData )
{
...
void QrSlam::saveInitFrameDiff(const Point3f frame_diff, ofstream&  file )
{
...
//=====================坐标变换==================
void QrSlam::systemTransforeNew(Mat& sys_state, Point3f trans)
{
...
OdomMessage QrSlam::odomToWorld(const OdomMessage src)
{
    OdomMessage dst;
    dst.init(src);
...
Point2f QrSlam::worldToCamera(Point3f observe_increase_world)
{
...
Point2f QrSlam::cameraToImage(Point2f observe_increase_camera, int id)
{
...
bool QrSlam::frameFixedTransform(Mat& miu_state , const int fixed_mark_id)   //当前系统状态 涉及到的地图与初始原点的对比
{
 ...
 * @param angle
 */
float QrSlam::angleWrap(float& angle)
{
